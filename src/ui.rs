...
fn handle_button_click(event: &str) {
    match event {
        ADD_EVENT_BUTTON_EVENT => {
            // 添加事件逻辑
            tracing::info!("添加事件: {:?}", ui_state().read().unwrap_or_else(|poisoned| poisoned.into_inner()).event_data);
        },
        ...
        CHANGE_EVENT_BUTTON_EVENT => {
            // 修改事件逻辑
            tracing::info!("修改事件: {:?}", ui_state().read().unwrap_or_else(|poisoned| poisoned.into_inner()).modify_event_data);
        },
        DELETE_EVENT_BUTTON_EVENT => {
            // 删除事件逻辑
            tracing::info!("删除事件: {:?}", ui_state().read().unwrap_or_else(|poisoned| poisoned.into_inner()).selected_event_index);
        },
        TAB_ADD_EVENT => {
            // 切换到添加事件标签页
...
fn build_add_event_ui(state: &UiState) -> ui::Element {
    ...
    // 事件时间输入
    let event_time_label = ui::Element::new(ui::ElementType::P, Some("输入事件目标日（格式必须为YYYY-MM-DD）"))
        .size(16)
        .margin_bottom(8);
    ...
    let on_index_label = ui::Element::new(ui::ElementType::P, Some("是否显示在主页"))
        .size(16)
        .margin_bottom(8);
    let on_index_dropdown = ui::Element::new(ui::ElementType::Button, Some("是"))
        .on(ui::Event::Click, ON_INDEX_CHANGE_EVENT)
        .radius(8)
        .padding(12)
        .bg("#2A2A2D")
        .width_full();
    ...
    let if_staring_day_label = ui::Element::new(ui::ElementType::P, Some("是否计入起始日"))
        .size(16)
        .margin_bottom(8);
    let if_staring_day_dropdown = ui::Element::new(ui::ElementType::Button, Some("否"))
        .on(ui::Event::Click, IF_STARING_DAY_CHANGE_EVENT)
        .radius(8)
        .padding(12)
        .bg("#2A2A2D")
        .width_full();
    ...
    // 组合添加事件界面
    container
        .child(ui::Element::new(ui::ElementType::Div, None).child(event_name_label).child(event_name_input))
        .child(ui::Element::new(ui::ElementType::Div, None).child(event_time_label).child(event_time_input))
        .child(ui::Element::new(ui::ElementType::Div, None).child(on_index_label).child(on_index_dropdown))
        .child(ui::Element::new(ui::ElementType::Div, None).child(if_staring_day_label).child(if_staring_day_dropdown))
...
fn build_modify_event_ui(state: &UiState) -> ui::Element {
    ...
    let event_name_label = ui::Element::new(ui::ElementType::P, Some("输入事件名称"))
        .size(16)
        .margin_bottom(8);
    let event_name_input = ui::Element::new(ui::ElementType::Input, Some(&state.modify_event_data.name))
    ...
    let event_time_label = ui::Element::new(ui::ElementType::P, Some("输入事件目标日（格式必须为YYYY-MM-DD）"))
        .size(16)
        .margin_bottom(8);
    let event_time_input = ui::Element::new(ui::ElementType::Input, Some(&state.modify_event_data.time))
    ...
    let on_index_label = ui::Element::new(ui::ElementType::P, Some("是否显示在主页"))
        .size(16)
        .margin_bottom(8);
    let on_index_dropdown = ui::Element::new(ui::ElementType::Button, Some(if state.modify_event_data.on_index { "是" } else { "否" }))
        .on(ui::Event::Click, MODIFY_ON_INDEX_CHANGE_EVENT)
        .radius(8)
        .padding(12)
        .bg("#2A2A2D")
        .width_full();
    ...
    let if_staring_day_label = ui::Element::new(ui::ElementType::P, Some("是否计入起始日"))
        .size(16)
        .margin_bottom(8);
    let if_staring_day_dropdown = ui::Element::new(ui::ElementType::Button, Some(if state.modify_event_data.if_staring_day { "是" } else { "否" }))
        .on(ui::Event::Click, MODIFY_IF_STARING_DAY_CHANGE_EVENT)
    ...
    // 组合修改事件界面
    container
        .child(ui::Element::new(ui::ElementType::Div, None).child(select_event_label).child(select_event_dropdown))
        .child(ui::Element::new(ui::ElementType::Div, None).child(event_name_label).child(event_name_input))
        .child(ui::Element::new(ui::ElementType::Div, None).child(event_time_label).child(event_time_input))
        .child(ui::Element::new(ui::ElementType::Div, None).child(on_index_label).child(on_index_dropdown))
        .child(ui::Element::new(ui::ElementType::Div, None).child(if_staring_day_label).child(if_staring_day_dropdown))
